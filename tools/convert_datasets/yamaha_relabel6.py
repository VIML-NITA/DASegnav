...
with open(osp.join(rellis_dir, 'train.txt'), 'r') as r:
    ...
    for l in r:
        ...
        img_bytes = file_client.get(rellis_dir + annotation_folder + "train/" +l.strip() + '.jpg')
        ...
        mmcv.imwrite(out1, rellis_dir + annotation_folder + "train/" +l.strip() + "_orig.png")
        mmcv.imwrite(out2, rellis_dir + annotation_folder + "train/" +l.strip() + "_group6new.png")
...
with open(osp.join(rellis_dir, 'val.txt'), 'r') as r:
    ...
    for l in r:
        ...
        file_client_args=dict(backend='disk')
        file_client = mmcv.FileClient(**file_client_args)
        img_bytes = file_client.get(rellis_dir + annotation_folder + "val/" +l.strip() + '.jpg')
        gt_semantic_seg = mmcv.imfrombytes(img_bytes, flag='unchanged', backend='pillow').squeeze().astype(np.uint8)
        out1, out2 = raw_to_seq(gt_semantic_seg)

        mmcv.imwrite(out1, rellis_dir + annotation_folder + "val/" +l.strip() + "_orig.png")
        mmcv.imwrite(out2, rellis_dir + annotation_folder + "val/" +l.strip() + "_group6new.png")
